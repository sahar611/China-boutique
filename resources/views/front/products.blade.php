  ...
  <input type="hidden" name="to" id="toHidden" value="{{ $priceTo ?? '' }}">
        ...
        >
                                                            ...
                                                            <label class="form-check-label" for="cat_{{ $cat->id }}">
                                                                {{ app()->getLocale()==='ar' ? $cat->name_ar : $cat->name_en }}<span>{{ $cat->products_count ?? '' }}</span>
                                                            </label>
                                                        </div>
                                                    </li>
                                                   @endforeach
                                    ...
                                    <div class="widget-content">
                                        ...
                                        <div class="price-inputs-wrapper">
                                            <div class="row g-2">
                                                ...
                                                <input type="number" class="form-control" id="priceFrom" placeholder="0" min="0" value="{{ $priceFrom ?? '' }}">                                                </div>
                                                <div class="col-6">
                                                    <label for="priceTo" class="form-label">To</label>
                                                 <input type="number" class="form-control" id="priceTo" placeholder="1000" min="0" value="{{ $priceTo ?? '' }}">                                                </div>
                                            </div>
                                            ...
                                            <div class="dropdown-menu-custom" id="brandsDropdownMenu">
                                                <ul class="categories-list">
                                                   @foreach($brands as $br)
                                                    <li>
                                                        <div class="form-check">
 <input class="form-check-input brand-checkbox" type="checkbox"  name="brand_ids[]" value="{{ $br->id }}" id="br_{{ $br->id }}" form="filtersForm"@checked(in_array($br->id, $selectedBrandIds)) >                                                            
  ...
  <option value="new" @selected(($sort ?? 'new')==='new')>Sort by Newness</option>
...
<img src="{{ $img ? asset($img) : asset('assets/images/placeholder.png') }}" alt="{{ $product->name_en ?? 'Product' }}">
                                                ...
                                                <div class="cart-button">
                                                     <a href="javascript:void(0)" class="cart-btn js-add-to-cart" data-url="{{ route('cart.add', $product->slug) }}" data-qty-input=".quantity"><i class="far fa-shopping-basket"></i>
...
<div class="product-price">
  @if($hasSale)
    <span class="price prev-price"><span class="currency">{{ $symbol }}</span>{{ number_format($oldDisplay, 2) }}</span>
    <span class="price new-price"><span class="currency">{{ $symbol }}</span>{{ number_format($priceDisplay, 2) }}</span>
  @else
    <span class="price new-price"><span class="currency">{{ $symbol }}</span>{{ number_format($priceDisplay, 2) }}</span>
  @endif
</div>
